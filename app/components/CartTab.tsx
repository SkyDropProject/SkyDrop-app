...
const CartTab = (): ReactElement => {
    ...
    const totalPrice = products.reduce((sum, product) => sum + ((product.price || 0) * (product.quantity || 1)), 0);
    ...
    const handleOrder = async (): Promise<void> => {
        ...
        const userId = await AsyncStorage.getItem('userId');
        const coords = AsyncStorage.getItem('location');
        const payload = {
            products: products,
            price: totalPrice,
            coordinates: coords
        ...
        <View style={styles.cartmenu}>
            ...
            <View style={styles.header}>
                <TitleText
                    size={TitleSize.h3}
                    text={
                        intl.formatMessage({ id: 'article_title' }) + ' (' + totalItems + ')'
                    }
                />
            </View>
